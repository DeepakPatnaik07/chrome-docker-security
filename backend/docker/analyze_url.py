...
def get_domain(url):
    try:
        ...
        if len(parts) >= 2:
            # Handle cases like www.google.com -> google.com and
            # login.google.co.uk -> google.co.uk (basic TLD handling)
...
async def run(playwright):
    ...
    try:
        ...
        page.on(
            "response",
            lambda response: (
                results["redirects"].append(response.url)
                if response.status >= 300 and response.status < 400
                else None
                # TODO: Consider capturing redirect chain details (e.g., final
                # destination URL vs original). Note: Playwright's response
                # object might not directly give the full chain.
            ),
        )
        ...
        try:
            ...
            # Ensure content is treated as string, take first 500 chars
            preview = str(content or "")
            results["page_content_preview"] = preview[:500]
        ...
        if original_hostname and is_ip_address(original_hostname):
            ...
            results["analysis"]["reasons"].append(
                f"URL uses direct IP address: {original_hostname}"
            )
        else:
        ...
        try:
            ...
            if meta_refresh:
                ...
                if content_attr and content_attr.strip().startswith(
                    "0;"
                ):  # Check if refresh is immediate (0 seconds)
                    results["analysis"]["suspicious"] = True
                    results["analysis"]["reasons"].append(
                        "Immediate meta refresh detected: " f"content='{content_attr}'"
                    )
                else:
                    results["analysis"][
                        "local_checks_passed"
                    ] += 1  # Count non-immediate refresh as passed
        except Exception as e:
            print(
                f"Error checking meta refresh: {e}"
        ...
        if original_domain and not is_ip_address(original_hostname):
            ...
            if len(subdomain_parts) > 5:
                results["analysis"]["suspicious"] = True
                results["analysis"]["reasons"].append(
                    f"Excessive subdomains detected ({len(subdomain_parts)} parts)"
                )
            else:
                results["analysis"]["local_checks_passed"] += 1
            # Brand name mismatch?
            ...
            for brand in common_brands:
                ...
                if brand in original_hostname and brand not in original_domain:
                    ...
                    results["analysis"]["suspicious"] = True
                    results["analysis"]["reasons"].append(
                        f"Brand mismatch: '{brand}' in subdomain/hostname "
                        f"but not main domain."
                    )
        ...
        for req_url in results["requests"]:
            ...
            try:
                ...
                if tld in suspicious_tlds:
                    results["analysis"]["suspicious"] = True
                    reason = f"Request to suspicious TLD ({tld}): " f"{req_url}"
                    results["analysis"]["reasons"].append(reason)
            except Exception:
        ...
        if len(external_domains) > 10:
            results["analysis"]["suspicious"] = True
            results["analysis"]["reasons"].append(
                "High number of unique external domains requested: "
            ...
            for frame in iframes[1:]:
                try:
                    ...
                    if (
                        ...
                        and not frame_url.startswith("data:")
                    ):
                        ...
                        if (
                            frame_domain
                            and original_domain
                            and frame_domain != original_domain
                        ):
                            if not frame_domain.endswith(original_domain):
                                results["analysis"]["suspicious"] = True
                                results["analysis"]["reasons"].append(
                                    f"External domain iframe detected: {frame_url}"
                                )
                    else:
        ...
        if redirect_count > 5:
            # Flagging excessive redirects might be too noisy, consider adjusting threshold
            # results["analysis"]["suspicious"] = True
            # For now, just record the reason if excessive
            results["analysis"]["reasons"].append(
                f"Excessive redirects: {redirect_count}"
            )
        else:
            results["analysis"]["local_checks_passed"] += 1
        ...
        try:
            ...
            if password_input:
                results["analysis"]["suspicious"] = True
                results["analysis"]["reasons"].append(
                    "Password input field found on page."
                )
            else:
                results["analysis"]["local_checks_passed"] += 1
        except Exception as e:
        ...
        checks_to_run += 1
        try:
            # Increase timeout slightly for inner_text, can be slow
            page_text = await page.locator("body").inner_text(timeout=7000)
            page_text_lower = page_text.lower()
            print(
                f"DEBUG: Page text (first 200 chars): {page_text_lower[:200]}"
            )  # DEBUG LOG - Wrap this line if needed
            found_keywords = set()
            ...
            if len(found_keywords) > 1:
                results["analysis"]["suspicious"] = True
                kw_list = list(found_keywords)
                results["analysis"]["reasons"].append(f"Keywords found: {kw_list}")
            else:
                results["analysis"]["local_checks_passed"] += 1
        except Exception as e:
