...
print(
    f"*** Python Check: os.environ.get('GOOGLE_API_KEY') = "
...
if not GOOGLE_API_KEY:
    print(
        "Warning: GOOGLE_API_KEY environment variable not set. AI analysis will be skipped."
    )
...
async def analyze_with_gemini(local_analysis: dict) -> dict:
    ...
    try:
        ...
        prompt = (
            "You are a security assistant evaluating a URL for a non-technical user.\\n"
        )
        prompt += "Analyze the following web page data for potential phishing or security risks.\\n"
        ...
        if local_analysis.get("analysis", {}).get("suspicious"):
            reasons = local_analysis.get("analysis", {}).get("reasons", [])
            # Ensure reasons is a string for the f-string
            reasons_str = str(reasons)
            prompt += f"Local analysis flagged these potential issues: {reasons_str}\n"
        else:
            prompt += "Local analysis did not find obvious suspicious indicators.\n"
        redirects = local_analysis.get("redirects", [])
        prompt += f"Number of redirects: {len(redirects)}\n"
        ...
        prompt += (
            "Followed by a single, very brief sentence explaining the main reason "
            "in plain language suitable for a regular computer user."
        )
        # --- End Revised Prompt ---
        ...
        return {"ai_skipped": False, "ai_assessment": ai_response_text}
...
async def analyze_link(link: Link):
    ...
    try:
        ...
        container.wait()
        logs = container.logs(stream=False, stdout=True, stderr=True).decode(
            "utf-8", errors="ignore"
        )
    ...
    proceed = local_analysis_result.get(
        "status"
    ) != "error" or local_analysis_result.get("url")
    if proceed:
